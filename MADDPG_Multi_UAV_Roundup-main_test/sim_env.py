...
class UAVEnv:
    def __init__(self, length=2, num_obstacle=3, num_agents=4):
        ...
        # 初始化卡尔曼滤波器
        self.kalman_filters = [KalmanFilter(0.1, 1e-4, 1e-2) for _ in range(self.num_agents)]
    ...
    def reset(self):
        ...
        for i in range(self.num_agents):
            ...
            # 初始化卡尔曼滤波器的状态
            self.kalman_filters[i].x = np.array([self.multi_current_pos[i][0], self.multi_current_pos[i][1], 0, 0])
    ...
    def kalman_predict(self):
        """使用卡尔曼滤波器预测所有智能体的下一时刻位置"""
        predicted_positions = []
        for i in range(self.num_agents):
            # 获取当前测量值（当前位置）
            z = self.multi_current_pos[i]

            # 更新卡尔曼滤波器
            self.kalman_filters[i].update(z)

            # 预测下一时刻的位置
            predicted_pos = self.kalman_filters[i].predict()
    ...
    def cal_rewards_dones(self, IsCollied, last_d):
        ...
        # 初始化各部分奖励
        r_near_rewards = np.zeros(self.num_agents)
        r_safe_rewards = np.zeros(self.num_agents)
        r_multi_stage_rewards = np.zeros(self.num_agents)
        r_finish_rewards = np.zeros(self.num_agents)
        r_smooth_rewards = np.zeros(self.num_agents)

        # 1. 单个无人机接近目标的奖励
        for i in range(3):  # 只计算前三个无人机
            pos = self.multi_current_pos[i]
            vel = self.multi_current_vel[i]
            pos_target = self.multi_current_pos[-1]
            v_i = np.linalg.norm(vel)
            dire_vec = pos_target - pos
            d = np.linalg.norm(dire_vec)  # 距离目标
            r_near = min(abs(v_i / self.v_max) * 1.0 / (d + 1e-5), 10) / 5
            r_near_rewards[i] = mu1 * r_near
        ...
        # 阶段奖励
        r_multi_stage_rewards[-1] = np.clip(10 * (Sum_d - Sum_last_d), -2, 2)  # 对目标无人机的奖励
        if Sum_S > S4 and Sum_d >= d_limit and all(d >= d_capture for d in [d1, d2, d3]):
            r_track = -Sum_d / max([d1, d2, d3])
            r_multi_stage_rewards[0:2] += mu3 * r_track
        elif Sum_S > S4 and (Sum_d < d_limit or any(d >= d_capture for d in [d1, d2, d3])):
            r_encircle = -1 / 3 * np.log(Sum_S - S4 + 1)
            r_multi_stage_rewards[0:2] += mu3 * r_encircle
        elif Sum_S == S4 and any(d > d_capture for d in [d1, d2, d3]):
            r_capture = np.exp((Sum_last_d - Sum_d) / (3 * self.v_max))
            r_multi_stage_rewards[0:2] += mu3 * r_capture

        # 4. 完成奖励
        if Sum_S == S4 and all(d <= d_capture for d in [d1, d2, d3]):
            r_finish_rewards[0:2] += mu4 * 10
            dones = [True] * self.num_agents

        # 使用卡尔曼滤波预测下一时刻的位置
        predicted_positions = self.kalman_predict()

        # 更新历史位置
        for i in range(self.num_agents):
            self.history_positions[i].append(predicted_positions[i])
            if len(self.history_positions[i]) > 2:
                self.history_positions[i].pop(0)

        # 5 计算平滑性奖励
        r_smooth_rewards = np.zeros(self.num_agents) # TODO 修改速度预测部分，使得加速度预测引入平滑项目
        for i in range(self.num_agents-1):
            if len(self.history_positions[i]) >= 2:
                last_vel = self.multi_current_vel[i]
                prev_vel = (self.history_positions[i][-1] - self.history_positions[i][-2]) / self.time_step
                acc = (last_vel - prev_vel) / self.time_step
                r_smooth = -alpha * np.linalg.norm(acc) ** 2
                r_smooth_rewards[i] = mu5 * r_smooth

                # 调试输出
                print(f"Agent {i}:")
                print(f"  last_vel: {last_vel}")
                print(f"  prev_vel: {prev_vel}")
                print(f"  acc: {acc}")
                print(f"  r_smooth: {r_smooth}")
                print("-------------------------------------------------")

        # 最终奖励累加
        rewards += (r_near_rewards + r_safe_rewards + r_multi_stage_rewards + r_finish_rewards + r_smooth_rewards)

        # 输出各部分奖励（可选）
        # print(f"r_near_rewards: {r_near_rewards}")
        # print(f"r_safe_rewards: {r_safe_rewards}")
        # print(f"r_multi_stage_rewards: {r_multi_stage_rewards}")
        # print(f"r_finish_rewards: {r_finish_rewards}")
        # print(f"r_smooth_rewards: {r_smooth_rewards}")
        # print("-----------------------------------------------------------------")
